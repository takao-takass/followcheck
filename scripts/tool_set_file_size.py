...
class SetFileSize:
    ...
    def run():
        ...
        try:
            # スレッドIDの発行
            thread_id = thread.ThreadId().CreateThread('set_file_size.py', 1)
            log = logger.ThreadLogging(thread_id)
            db = databeses.DbConnection(log)

            filesize_empty_meadias = db.fetch(
                " SELECT tm.service_user_id, tm.user_id, tm.tweet_id, tm.url, tm.directory_path, tm.file_name "
            ...
            for filesize_empty_meadia in filesize_empty_meadias:
                ...
                file_path = filesize_empty_meadia['directory_path'] + filesize_empty_meadia['file_name']
                file_size = -1
            ...
            if 'thread_id' in locals():
                log.info('プロセスを終了します。')
                thread.ThreadId().ExitThread('set_file_size.py', thread_id)
